...
export async function sendEmail(env: Env, options: EmailOptions): Promise<void> {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "Test Project 1 <noreply@test-project-1.com>",
      to: options.to,
...
export function welcomeEmailTemplate(name: string): { subject: string; html: string } {
  return {
    subject: "Welcome to Test Project 1!",
    html: `
      <h1>Welcome, ${name || "there"}!</h1>
      <p>Thank you for signing up for Test Project 1.</p>
      <p>Get started by exploring your dashboard.</p>
      <a href="https://test-project-1.spinitup.dev/dashboard">Go to Dashboard</a>
    `,
  };
}
